...
mutex mtx;
CLogBuffer buff = CLogBuffer(mtx);

void BlurProc(ThreadInfo* thread, int startBlur, int endBlur, clock_t startTime)
{
	...
	for (int i = 0; i < w; i++)
	{
		for (int j = startBlur + 2; j < endBlur - 2; j++)
		{
			...
			buff.Push(to_string(thread->index) + " " + to_string(difftime(endTime, thread->startTime)));
		}
	}
}
...
DWORD WINAPI ThreadProc(CONST LPVOID lpParam)
{
	...
	BlurProc(thread, startBlurIndex, endBlurIndex, startTime);
	ExitThread(0);
}
...
void CBlur::StartBluring(clock_t start) {
	BMP inputImage;
